...
export interface ITradeRequest extends Document {
  ...
  status: 'pending' | 'negotiating' | 'approved' | 'funded' | 'executed' | 'cancelled';
...
const TradeRequestSchema = new Schema<ITradeRequest>(
  {
    ...
    status: {
      type: String,
      required: true,
      enum: ['pending', 'negotiating', 'approved', 'funded', 'executed', 'cancelled'],
      default: 'pending',
    },
...
export const TradeRequest = mongoose.models.TradeRequest || mongoose.model<ITradeRequest>('TradeRequest', TradeRequestSchema);
